...
class Hotspot:
    ...
    def update(self, fingertips: list[tuple[float, float]]) -> None:
        ...
        if self._prev_fingertip_inside and not fingertip_inside:
            self._event_handler.on_hotspot_unpressed(self.id)
        ...
        if not self._prev_fingertip_inside and fingertip_inside:
            self._event_handler.on_hotspot_pressed(self.id)
    ...
    def _is_point_inside(self, point: tuple[float, float]) -> bool:
        ...
        """
        squared_dist = (self._norm_pos[0] - point[0]) ** 2 + (self._norm_pos[1] - point[1]) ** 2
