...
class Calibrator:
    ...
    def calibrate(projector_id_to_coord: dict[int, tuple[float, float]]) -> np.ndarray:
        # returns a 3x3 array of 32bit floats
...
def calibrate(projector_id_to_coord_str: str):  # TODO typing returns a clr system array
    projector_id_to_coord_temp: dict[str, tuple[float, float]] = json.loads(projector_id_to_coord_str)
    projector_id_to_coord: dict[int, tuple[float, float]] = {int(k): v for k, v in projector_id_to_coord_temp.items()}
